  ...
  return (
    <div
      ...
      className={`relative group cursor-pointer flex flex-col p-5 rounded-3xl transition-all duration-300 ${
        isActive
          ? 'bg-white shadow-xl shadow-slate-900/10 border-2 border-slate-900 scale-[1.02]'
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-2 mt-auto">
        <div className="bg-slate-50 rounded-xl p-2 flex flex-col">
          <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wide">Meds</span>
          <span className="text-sm font-black text-slate-800">{profile.medsCount || 0} active</span>
        </div>
        <div className="bg-slate-50 rounded-xl p-2 flex flex-col">
          ...
          {profile.alerts ? (
            <span className="text-sm font-black text-red-600 flex items-center gap-1">
              <ShieldAlert className="w-3.5 h-3.5" /> {profile.alerts} alerts
            </span>
          ) : (
...
export const FamilySection = ({ profiles, activeProfile, onProfileSelect }) => {
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-slate-900 hover:bg-slate-800 active:scale-95 text-white text-sm font-bold shadow-md shadow-slate-900/10 transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
        ...
        {/* Empty Add Card */}
        <button className="flex flex-col items-center justify-center gap-3 min-h-[200px] p-5 rounded-3xl border-2 border-dashed border-slate-200 hover:border-indigo-300 hover:bg-indigo-50/30 transition-all group">
