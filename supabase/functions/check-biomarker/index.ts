...
/**
 * Biomarker Checking & Verification Edge Function
 * Reviews unmapped laboratory test items from reports against the master catalog.
 * Discards hospital disclaimers/noise, maps unindexed synonyms to existing biomarkers,
 * or registers new genuine medical biomarkers with complete clinical interpretation profiles.
 */
...
- interpretation_low: Clinical meaning of values below reference range (e.g. deficiency, organ failure, anemia).
- interpretation_high: Clinical meaning of values above reference range (e.g. toxicity, inflammation, organ injury).
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { unmapped_items, auto_persist = true } = body;

    if (!unmapped_items || !Array.isArray(unmapped_items) || unmapped_items.length === 0) {
      return new Response(
        JSON.stringify({ error: 'unmapped_items array is required and must not be empty.' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    // 1. Fetch active master biomarkers and aliases from database
    let existingBiomarkers = body.existing_biomarkers;
    if (!existingBiomarkers || !Array.isArray(existingBiomarkers)) {
      ...
      existingBiomarkers = (masters || []).map((m: any) => ({
        biomarker_id: m.biomarker_id,
        standard_name: m.standard_name,
        display_name: m.display_name,
        unit: m.unit,
        category: m.category,
        aliases: (m.biomarkers_aliases || []).map((a: any) => a.alias_name),
      }));
    }
...
`;
    ...
    // Strict model priority as specified by user:
    ...
    // Priority API Key: GEMINI_API_KEY_COMMON, with GEMINI_API_KEY_REPORT fallback
    const candidateKeys = [
    ...
    const aiResult = await generateClinicalAI({
      prompt: fullPrompt,
      candidateKeys,
      candidateModels,
    });
    ...
    if (auto_persist && Array.isArray(decisions)) {
      for (const item of decisions) {
        if (item.action === 'MAP_TO_EXISTING') {
          ...
          // Insert into biomarkers_mastertable
          ...
          // Prepare aliases
          const rawAliases: string[] = Array.isArray(nb.aliases) ? nb.aliases : [];
    ...
    console.error('Check Biomarker Edge Function Error:', error);
    return new Response(
